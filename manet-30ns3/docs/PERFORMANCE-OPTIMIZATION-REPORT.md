...
```
Container-0 eth0 → mesh-veth0 → mesh-br-0 → mesh-tap-0 → [ns-3 进程] → mesh-tap-1 → mesh-br-1 → mesh-veth1 → Container-1
```
...
**问题**:
- **TAP 设备归属**: `mesh-tap-0` 到 `mesh-tap-7` 由进程 A 打开，`mesh-tap-8` 到 `mesh-tap-15` 由进程 B 打开。Linux 不允许两个进程同时 `open()` 同一个 TAP 设备（除非使用 `TUNSETIFF` + `IFF_MULTI_QUEUE`，但 ns-3 TapBridge 不支持）。
